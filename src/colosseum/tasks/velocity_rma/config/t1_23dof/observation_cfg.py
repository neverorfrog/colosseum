"""Observations for t1-velocity-rma.

Actor/critic terms are copied from velocity_symmetric so noise levels and mirror
functions are consistent with the reference policy.  The 'env_params' privileged
group exposes all DR-randomized physics parameters to the RMA encoder (9D total):

  foot_friction       (1D) — tangential friction coefficient for foot geoms
  base_com            (3D) — base body COM offset from parent frame origin
  kp_scale            (1D) — mean PD proportional gain scale
  kd_scale            (1D) — mean PD derivative gain scale
  link_mass_scale     (1D) — mean body mass scale (pseudo_inertia DR)
  joint_damping_scale (1D) — mean joint damping scale
  effort_limit_scale  (1D) — mean actuator effort limit scale
"""
...
from colosseum.tasks.velocity_rma.mdp.observations import (
...
# ---------------------------------------------------------------------------
# Actor / critic terms — copied from velocity_symmetric for noise consistency
# ---------------------------------------------------------------------------
