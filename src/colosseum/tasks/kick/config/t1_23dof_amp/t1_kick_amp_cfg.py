"""Residual AMP kick task: frozen walk branch + trainable residual branch + AMP.
...
``ball_angle`` (BallAngleCommand) gives the residual a fixed per-episode kick
direction. The observation manager packs five groups (``loco_actor``,
``kick_actor``, ``orchestrator``, ``critic``, ``amp``) consumed by
ResidualAMPPPO; see observation_cfg.py and algo_cfg.py.

The step-0 validation (frozen walk steered by the ball-as-twist trick) can
still be run with a velocity checkpoint:

    pixi run play task:t1-kick \
      --checkpoint models/t1-velocity/jun18_1/t1-velocity_ppo_jun18_1.pt --debug-vel
"""
...
from .algo_cfg import booster_t1_kick_amp_ppo_cfg
...
def booster_t1_kick_env_cfg(play: bool = False) -> ColosseumEnvCfg:
  cfg = ColosseumEnvCfg(
  ...
  if play:
    ...
    # assert isinstance(ball_angle, BallAngleCommandCfg)
...
class T1KickAmpTask(TaskConfig):
  name: str = "t1-kick-amp"
  env: ColosseumEnvCfg = field(default_factory=booster_t1_kick_env_cfg)

  @property
  def train_env_cfg(self):
    return booster_t1_kick_env_cfg(play=False)

  @property
  def play_env_cfg(self):
    return booster_t1_kick_env_cfg(play=True)

  @property
  def algo_cfg(self):
    return booster_t1_kick_amp_ppo_cfg()
