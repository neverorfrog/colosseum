"""booster_gym-mimic velocity task on the 12-DOF (legs-only) T1 model."""
...
from colosseum.robots.t1_12dof.constants import BASE_BODY_NAME, get_robot_cfg
from colosseum.robots.t1_12dof.sensors import (
...
from .algo_cfg import booster_t1_12dof_ppo_cfg
...
def booster_t1_12dof_velocity_env_cfg(play: bool = False) -> ColosseumEnvCfg:
  cfg = ColosseumEnvCfg(
...
def get_t1_12dof_velocity_checkpoint(version: int) -> Path:
  version_dir = project_root() / "models" / "t1-velocity-12dof" / f"v{version}"
...
@register_task("t1-velocity-12dof")
@dataclass(frozen=True)
class T1Velocity12dofTask(TaskConfig):
  name: str = "t1-velocity-12dof"
  env: ColosseumEnvCfg = field(default_factory=booster_t1_12dof_velocity_env_cfg)
  ...
  def play_env_cfg(self):
    return booster_t1_12dof_velocity_env_cfg(play=True)

  @property
  def algo_cfg(self):
    return booster_t1_12dof_ppo_cfg()
