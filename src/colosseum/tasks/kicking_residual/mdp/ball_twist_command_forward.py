...
Command convention (body frame, matching the walk policy's training):
    ...
    vx    = speed * max(dir.x, 0), i.e. walk forward only once roughly facing the ball
    vy    = 0, no lateral command: turn to face the ball instead of crab-walking
...
class BallForwardTwistCommand(CommandTerm):
  """Body-frame twist [vx, vy, wz] turning the robot to face the ball, then walking forward."""

  cfg: BallForwardTwistCommandCfg

  def __init__(self, cfg: BallForwardTwistCommandCfg, env: ManagerBasedRlEnv):
  ...
  def _update_command(self) -> None:
    ...
    # No lateral command: walk forward only once roughly facing the ball,
    # otherwise turn in place (wz below). Avoids commanding crab-walking.
    forward = direction[:, :1].clamp(min=0.0)
    self.twist[:, :1] = speed * forward
    self.twist[:, 1] = 0.0
  ...
  def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
    ...
    visualizer.add_arrow(
      ...
      label=f"ball_forward_twist |v|={self.twist[batch, :2].norm():.2f}",
    )


@dataclass(kw_only=True)
class BallForwardTwistCommandCfg(CommandTermCfg):
  """Configuration for BallForwardTwistCommand."""

  class_type: type[CommandTerm] = BallForwardTwistCommand
  ...
  # Map ball distance -> forward speed, clipped to the walk policy's range.
  ...
  def build(self, env: ManagerBasedRlEnv) -> BallForwardTwistCommand:
    return BallForwardTwistCommand(self, env)
