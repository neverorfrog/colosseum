...
direction. The observation manager packs four groups (``loco_actor``,
``kick_actor``, ``orchestrator``, ``critic``) consumed by ResidualPPO; see
observation_cfg.py.

The step-0 validation (frozen v4 walk steered by the ball-as-twist trick) can
still be run with the velocity checkpoint:

    pixi run play task:t1-kicking-residual \
...
def booster_t1_kicking_residual_env_cfg(play: bool = False) -> ColosseumEnvCfg:
  cfg = ColosseumEnvCfg(
...
@register_task("t1-kicking-residual")
@dataclass(frozen=True)
class T1KickingResidualTask(TaskConfig):
  name: str = "t1-kicking-residual"
  env: ColosseumEnvCfg = field(default_factory=booster_t1_kicking_residual_env_cfg)

  @property
  def train_env_cfg(self):
    return booster_t1_kicking_residual_env_cfg(play=False)

  @property
  def play_env_cfg(self):
    return booster_t1_kicking_residual_env_cfg(play=True)
