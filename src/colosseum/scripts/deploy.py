...
def main(
  task: Annotated[str, tyro.conf.arg(aliases=["-t"])] = "t1-velocity-flat",
  backend: Annotated[
    Literal["mujoco", "robot", "webots"], tyro.conf.arg(aliases=["-b"])
  ] = "mujoco",
  ...
  if backend == "mujoco":
    ...
    print("[Deploy] Starting MuJoCo simulation...")
    print(f"[Deploy] Initial position: {config.mujoco.init_pos}")
    print(
      f"[Deploy] Decimation: {config.mujoco.decimation} (physics_dt={config.physics_dt:.4f}s)"
    )
  ...
  #     print("[Deploy] Starting real robot deployment...")
