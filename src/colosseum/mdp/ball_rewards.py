...
def ball_vel_tracking_body(
  ...
  tracking error too — but only the excess above ``vz_tolerance``: a hard kick
  inevitably lofts the ball a little, so small vz is free and only genuinely
  lifted balls add (vz - tol)² to the XY error inside the exp.
  """
  ball_vel_b = ball_vel_body(env)
  error_sq = ((ball_vel_b - cmd_body(env, command_name)) ** 2).sum(dim=-1)
  vz = env.scene["ball"].data.root_link_lin_vel_w[:, 2]
  vz_up = (vz - vz_tolerance).clamp(min=0.0)
