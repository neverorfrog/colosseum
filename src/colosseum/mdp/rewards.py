...
class pose_deviation_penalty:
    ...
    Uses regex patterns to assign weights to joints, matching against the
    entity's joint_names in natural order (no hardcoded indices needed).
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        ...
        _, _, weights = resolve_matching_names_values(
            data=cfg.params["pose_weights"],
            list_of_strings=joint_names,
        )
        self.weights = torch.tensor(weights, device=env.device, dtype=torch.float32)
