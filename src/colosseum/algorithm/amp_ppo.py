"""AmpPPO — PPO + an Adversarial Motion Prior (BeyondAMP-style, from scratch).

Vanilla PPO learns the full task (e.g. velocity-tracked walking) from scratch,
while an AMP discriminator supplies the *style*: an extra ``amp`` observation
group feeds a network trained to tell the policy's motion transitions apart from
a reference clip. The discriminator's score becomes a per-step reward, blended
with the task reward, replacing hand-tuned gait/pose shaping.

AMP plugs into exactly two seams the PPO base exposes:
  ...
  2. Learning — ``_update_discriminator``, run after the inherited PPO update:
     train the discriminator (LSGAN + gradient penalty) on policy vs. expert
     transitions.

The expert data only ever enters the discriminator loss; the reward path only
feeds the discriminator policy transitions. The two are coupled solely through
...
from colosseum.config.types.algorithm import AmpPpoConfig, register_algorithm


@register_algorithm("amp_ppo", config_class=AmpPpoConfig)
class AmpPPO(PPO):
  """PPO with an AMP discriminator providing an adversarial style reward."""
  ...
  def __init__(
    ...
    assert isinstance(self.config, AmpPpoConfig)
    cfg = self.config
    ...
    # Resolve the same joint filter the env's amp obs group uses, so the expert
    # dataset slices the matching columns (e.g. head excluded). None → all joints.
    amp_joint_ids: list[int] | None = None
    if cfg.amp_joint_names is not None:
      sec = SceneEntityCfg("robot", joint_names=cfg.amp_joint_names)
      sec.resolve(self.env.scene)
      amp_joint_ids = list(sec.joint_ids)
    ...
    self.amp_motion_dataset = AmpMotionDataset(
      ...
      include_base_vel=cfg.amp_include_base_vel,
      joint_ids=amp_joint_ids,
    )
