...
The keys are the name of the backend ("numpy", "torch") and the values are the corresponding type
...
The keys of the outer dictionary are the name of target backend ("numpy", "torch"). The keys of the
inner dictionary are the source backend (``np.ndarray``, ``torch.Tensor``).
"""
