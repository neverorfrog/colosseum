...
_RCLPY_IMPORT_ERROR: ImportError | None = None
_BOOSTER_INTERFACE_IMPORT_ERROR: ImportError | None = None
_BOOSTER_SDK_IMPORT_ERROR: ImportError | None = None

try:
  import rclpy  # type: ignore
  from rclpy.executors import (  # type: ignore
...
try:
  from booster_interface.msg import LowCmd, LowState, MotorCmd  # type: ignore
except ImportError as exc:
  LowState = None
  LowCmd = None
  MotorCmd = None
  _BOOSTER_INTERFACE_IMPORT_ERROR = exc
...
def _raise_missing_dependency(package: str, root_error: ImportError | None) -> None:
  message = (
    f"Package '{package}' is required to run the Booster deployment backend. "
    "Install it or disable this backend in your configuration."
  )
  if root_error is not None:
    raise ImportError(message) from root_error
  raise ImportError(message)


def _ensure_booster_runtime_dependencies() -> None:
  if rclpy is None:
    _raise_missing_dependency("rclpy", _RCLPY_IMPORT_ERROR)
  if LowState is None or LowCmd is None or MotorCmd is None:
    _raise_missing_dependency(
      "booster_interface",
      _BOOSTER_INTERFACE_IMPORT_ERROR,
    )
  if B1LocoClient is None or RobotMode is None:
    _raise_missing_dependency(
      "booster_robotics_sdk_python",
      _BOOSTER_SDK_IMPORT_ERROR,
    )
...
class BoosterRobotPortal:
  ...
  def __init__(self, cfg: ControllerConfig, use_sim_time: bool = False) -> None:
    _ensure_booster_runtime_dependencies()
    self.cfg = cfg
    ...
    self.timer = CountTimer(self.cfg.booster.low_state_dt, use_sim_time=use_sim_time)
  ...
  def _init_synced_buffer(self):
    ...
    self.synced_state = SyncedArray("state", shape=(1,), dtype=state_dtype)
  ...
  def _init_metrics(self):
    ...
    self.metrics = {
      "low_state_handler": SyncedMetrics("low_state_handler", max_events=max_events),
      "policy_step": SyncedMetrics("policy_step", max_events=max_events),
    }
    ...
    def low_state_service_executor():
      ...
      try:
        ...
        if is_rcl_error and is_shutting_down:
          ...
          self.logger.error(
            "Low state subscription executor stopped: %s", exc, exc_info=True
          )
      finally:
        executor.shutdown()
  ...
  def create_low_cmd_publisher(self, name):
    ...
    motor_cmd_buf = [MotorCmd() for _ in range(self.robot.num_joints)]  # type: ignore
  ...
  def cleanup(self) -> None:
    ...
    if self.low_cmd_process is not None and self.low_cmd_process.is_alive():
      ...
      if self.low_cmd_process.is_alive():
        self.logger.warning("Low cmd publisher process did not stop, terminating...")
...
class BoosterRobotController(BaseController):
  ...
  def __init__(self, cfg: ControllerConfig, portal: BoosterRobotPortal) -> None:
    super().__init__(cfg)
    self.portal = portal
  ...
  def update_state(self) -> None:
    ...
    self.robot.data.joint_pos = torch.from_numpy(state["joint_pos"]).to(
      dtype=torch.float32
    )
    self.robot.data.joint_vel = torch.from_numpy(state["joint_vel"]).to(
      dtype=torch.float32
    )
    self.robot.data.feedback_torque = torch.from_numpy(state["feedback_torque"]).to(
      dtype=torch.float32
    )
    self.robot.data.root_pos_w = torch.from_numpy(state["root_pos_w"]).to(
      dtype=torch.float32
    )
    rpy_t = torch.from_numpy(state["root_rpy_w"]).to(dtype=torch.float32)
    self.robot.data.root_quat_w = lab_math.quat_from_euler_xyz(*rpy_t).squeeze()
    self.robot.data.root_lin_vel_b = lab_math.quat_apply_inverse(
      self.robot.data.root_quat_w,
      torch.from_numpy(state["root_lin_vel_w"]).to(dtype=torch.float32),
    )
    self.robot.data.root_ang_vel_b = torch.from_numpy(state["root_ang_vel_b"]).to(
      dtype=torch.float32
    )
